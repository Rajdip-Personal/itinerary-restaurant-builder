...
export function insertSmartMealBreaks(
  ...
  for (const window of MEAL_WINDOWS) {
    ...
    if (itineraryEnd < window.startMin) {
      // Special case: itinerary ends before dinner but hotel is known —
      // traveler still needs dinner, suggest near hotel on the way back
      if (window.type === 'dinner' && hotelCoords) {
        const dinnerTime = minutesToTimeString(window.startMin);
        console.log(`[MealBreak] dinner: itinerary ends before dinner window, suggesting near hotel`);
        results.push({
          mealType: 'dinner',
          window: { start: mealWindow.start, end: mealWindow.end },
          candidates: [{
            ...
            cityId: timeline[timeline.length - 1].cityId,
          }],
        });
      }
      continue;
